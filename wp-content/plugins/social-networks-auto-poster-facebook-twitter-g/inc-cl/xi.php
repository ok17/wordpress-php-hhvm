  ...
  public function doAuth() { $ntInfo = $this->ntInfo; global $nxs_snapSetPgURL; if (isset($_GET['acc'])) $options = $this->nt[$_GET['acc']];
    if ( isset($_GET['auth']) && $_GET['auth']==$ntInfo['lcode']){
      ...
      switch ($tum_oauth->http_code) { case 201: case 200: $url = 'https://api.xing.com/v1/authorize?oauth_token='.$options['oAuthToken'];              
        if (function_exists('get_option')) $nxs_gNtwrks = get_option('nxsSNAPNetworks'); if(!empty($nxs_gNtwrks)) { $nxs_gNtwrks[$ntInfo['lcode']][$_GET['acc']] = $options; nxs_save_ntwrksOpts($nxs_gNtwrks); }
